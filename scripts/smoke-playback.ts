#!/usr/bin/env bun
/**
 * Absolute Cinema playback smoke test.
 *
 ...
 *   docker exec absolute-cinema bun /app/scripts/smoke-playback.ts
 *
 * Pre-bake (script not yet in image) — pipe via stdin:
 *   docker exec -i absolute-cinema bun - < scripts/smoke-playback.ts
 *
 * Env:
...
function printTable(report: SmokeReport): void {
  ...
  console.log("");
  console.log("=== Absolute Cinema smoke-playback ===");
