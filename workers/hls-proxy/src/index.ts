 ...
 *
 * - Requires a signed token minted by Absolute Cinema after user sign-in (HMAC-SHA256).
...
interface ProxyPayload {
  ...
  /** Expiry unix seconds */
  e: number;
  /** Absolute Cinema user id (audit / binding) */
  i: string;
}
...
function corsHeaders(request: Request, env: Env): Headers {
  ...
  if (origin && (allowed.length === 0 || allowed.includes(origin) || allowed.includes("*"))) {
    ...
    // Browser needs a value when Origin is sent from Absolute Cinema; allow first configured origin match fallthrough
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      ...
      return new Response(JSON.stringify({ ok: true, service: "absolute-cinema-hls-proxy" }), {
        headers: h,
      });
    }
    ...
    if (!token) {
      return new Response("Missing token — sign in to Absolute Cinema to play", {
        status: 401,
        headers: cors,
      });
    }
