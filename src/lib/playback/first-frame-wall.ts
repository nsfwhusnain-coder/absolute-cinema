...
/**
 * Cold start, no media data at all (`readyState === 0`). Four stalling 4K
 * HLS rows at the full 20s wall is 80s of black screen — the readyState-0
 * QA failure. A source that has not delivered a single byte by this point
 * is a dead CDN, not a slow Vixsrc playlist. Sources that *have* begun
 * buffering keep the full cold wall.
 */
export const FIRST_FRAME_WALL_NO_DATA_MS = 8_000;
...
/**
 * Failover budget when the element still has no media data.
 * Remux/sole-source keep the patient wall so we do not kill a packer or
 * the last remaining row. Multi-source cold starts cut over in 8s.
 */
export function firstFrameNoDataWallMs(opts: FirstFrameWallOpts): number {
  if (opts.remuxOrTranscode) return firstFrameWallMs(opts);
  const remaining = Math.max(0, opts.remainingSources | 0);
  if (remaining === 0) return firstFrameWallMs(opts);
  return FIRST_FRAME_WALL_NO_DATA_MS;
}
