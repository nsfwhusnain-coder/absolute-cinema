...
/** CinePro multi-provider streams (proxy path). */
...
export function scoreSource(source: PlaybackSource): number {
  const pref = typeof window !== "undefined" ? localStorage.getItem("absolute-cinema:preferred-provider") : null;
...
export function sortSourcesForPicker(sources: PlaybackSource[]): PlaybackSource[] {
  const pref =
    typeof window !== "undefined" ? localStorage.getItem("absolute-cinema:preferred-provider") : null;
