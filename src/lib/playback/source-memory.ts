...
function heightBand(source: PlaybackSource): number {
  return memoryHeightBand(source.maxHeight ?? 0);
}

/**
 * Coarse resolution bucket used both for ranking and for the durable key.
 * Unknown height ranks with HD so we never assume the worst (see source-quality).
 */
export function memoryHeightBand(height: number): number {
...
/**
 * Stable per-title memory key: server identity + resolution band.
 *
 * `source.id` includes a hash of the current URL (`sourceInstanceId`), so a
 * fresh HLS session would never match yesterday's row. Identity + band stays
 * true across URL rotation while keeping 1080 and 2160 as separate memories.
 */
export function memorySourceKey(source: PlaybackSource): string {
  return memorySourceKeyParts(
    source.provider,
    source.label,
    source.maxHeight ?? 0
  );
...
export function memorySourceKeyParts(
  provider: string,
  label: string,
  height: number
): string {
  return `${sourceIdentity(provider, label)}:h${memoryHeightBand(height)}`;
}
...
export function pickProvenSource<T extends PlaybackSource>(
  ...
  for (const source of sources) {
    rosterBand = Math.max(rosterBand, heightBand(source));
  }
  let best: T | null = null;
  let bestHeight = -1;
  for (const source of sources) {
    if (trustLevel(records.get(keyOf(source)), now) !== "proven") continue;
    // A fondly-remembered 1080p must never win the instant path when 4K is
    // sitting in the same roster. Defer to normal ranking in that case.
    if (heightBand(source) < rosterBand) continue;
