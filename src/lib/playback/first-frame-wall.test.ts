...
describe("firstFrameNoDataWallMs", () => {
  it("cuts a dead CDN over in 8s when another source remains", () => {
    expect(firstFrameNoDataWallMs({ resumeAt: 0, remainingSources: 3 })).toBe(
      FIRST_FRAME_WALL_NO_DATA_MS
    );
    expect(FIRST_FRAME_WALL_NO_DATA_MS).toBeLessThan(FIRST_FRAME_WALL_COLD_MS);
  });

  it("keeps the patient wall for a sole remaining source", () => {
    expect(firstFrameNoDataWallMs({ resumeAt: 0, remainingSources: 0 })).toBe(
      FIRST_FRAME_WALL_RESUME_MS
    );
  });

  it("does not kill a remux packer with the short wall", () => {
    expect(
      firstFrameNoDataWallMs({
        resumeAt: 0,
        remainingSources: 2,
