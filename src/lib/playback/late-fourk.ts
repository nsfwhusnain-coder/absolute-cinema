...
/**
 * Hunt a newly arrived 4K source unless the viewer explicitly capped below
 * Ultra. Auto and an unset profile follow the household default (4K).
 */
export function wantsFourKDiscovery(
  preferred: "auto" | number | null | undefined
): boolean {
  if (typeof preferred === "number" && preferred > 0 && preferred < UHD_HEIGHT) {
    return false;
  }
  return true;
}
