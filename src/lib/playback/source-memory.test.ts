...
describe("pickProvenSource", () => {
  ...
  it("does not pick a proven 1080p when any 4K is in the roster", () => {
    const hd = source({ id: "quasar", maxHeight: 1080 });
    const uhd = source({ id: "orion", maxHeight: 2160 });
    const records = new Map([
      ["quasar", provenRecord({ decodedHeight: 1080 })],
    ]);
    expect(pickProvenSource([hd, uhd], records, keyOf, NOW)).toBeNull();
  });
});

describe("memorySourceKey", () => {
  it("stays stable when the URL-hashed source.id rotates", () => {
    const first = source({
      id: "videasy-quasar-aaaaaaa",
      maxHeight: 2160,
    });
    const rotated = source({
      id: "videasy-quasar-bbbbbbb",
      maxHeight: 2160,
    });
    expect(memorySourceKey(first)).toBe(memorySourceKey(rotated));
    expect(memorySourceKey(first)).toBe("quasar:h3");
  });

  it("keeps 1080 and 2160 as separate memories for the same server", () => {
    const hd = source({ id: "q-hd", maxHeight: 1080 });
    const uhd = source({ id: "q-uhd", maxHeight: 2160 });
    expect(memorySourceKey(hd)).not.toBe(memorySourceKey(uhd));
  });
});
