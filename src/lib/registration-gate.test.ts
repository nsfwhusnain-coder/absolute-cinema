...
describe("checkRegistrationGate", () => {
  ...
  it("lets the first account register on a fresh install with no invite code", () => {
    const result = checkRegistrationGate({
      isAdminCreating: false,
      isFirstUser: true,
      requiredCode: undefined,
    ...
    expect(result).toEqual({ allowed: true, reason: "first_user" });
  });

  it("closes registration again once any account exists", () => {
    const result = checkRegistrationGate({
      isAdminCreating: false,
      isFirstUser: false,
      requiredCode: undefined,
