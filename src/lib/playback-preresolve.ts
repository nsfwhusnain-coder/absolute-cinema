...
export function buildPlaybackUrl(
  ...
  try {
    if (typeof window !== "undefined") {
      const raw = localStorage.getItem("absolute-cinema:preferred-quality");
      if (raw === "auto" || raw === null || raw === "") {
        ...
        params.set(
          "qualityHint",
          Number.isFinite(n) && [2160, 1080, 720, 480, 360].includes(n)
            ? String(n)
            : "auto"
        );
      }
...
function preferredManifestWarmHeight(): number {
  try {
    const stored = localStorage.getItem("absolute-cinema:preferred-quality");
