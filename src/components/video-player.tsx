...
import {
  firstFrameNoDataWallMs,
  firstFrameWallMs,
} from "@/lib/playback/first-frame-wall";
...
import { inferDynamicRange } from "@/lib/playback/stream-info";
...
interface Props {
  externalSubtitles?: Array<{ id: string; label: string; language: string; vttUrl: string; isDefault?: boolean }>;
...
export function VideoPlayer({
  ...
  */
  ...
  useEffect(() => {
    ...
    const wallOpts = { resumeAt, remainingSources, remuxOrTranscode };
    const wallMs = firstFrameWallMs(wallOpts);
    const noDataMs = firstFrameNoDataWallMs(wallOpts);
    const timer = window.setTimeout(() => {
      if (everPlayedRef.current) return;
      if (usePlayerStore.getState().error) return;
      failActiveSource("first_frame_timeout");
    }, wallMs);
    // Dead CDN: readyState stays 0. Four of those at 20s is 80s of black
    // screen. Sources that have begun buffering keep the full wall.
    const noDataTimer =
      noDataMs < wallMs
        ? window.setTimeout(() => {
            if (everPlayedRef.current) return;
    ...
    return () => {
      ...
      if (noDataTimer) window.clearTimeout(noDataTimer);
    };
  ...
  useEffect(() => {
    ...
    // no "Tap play to start" copy — center play button is enough
  ...
  const hdrDisplay = useMemo(
    () =>
      hdrSdrDisplayMap({
        dynamicRange: inferDynamicRange(activeSource, levels),
        userBrightness: videoBrightness,
      }),
    [activeSource, levels, videoBrightness]
  );
  ...
  return (
    ...
    >
      <video
        ...
        style={{ width: 320, height: 180, zIndex: -100 }}
        aria-hidden="true"
      />
      <video
        ref={videoRef}
        ...
        data-dynamic-range={hdrDisplay.shadowLift > 0 ? "hdr" : "sdr"}
        style={{
          position: "absolute",
          inset: 0,
          width: "100%",
      ...
      {/* Title + Paused overlaid on video — stays while paused, above controls */}
