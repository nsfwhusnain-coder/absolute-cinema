...
function isolatedDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "absolute-cinema-memory-"));
  dirs.push(dir);
...
describe("warm roster disk", () => {
  ...
  it("shares one roster file for fast/full of the same quality bucket", () => {
    ...
    expect(loadWarmRoster<RosterPayload>("movie:550:::q2160:full")?.result).toEqual({
      streamUrl: "https://cdn.test/q.m3u8",
      sources: [{ label: "Quasar" }],
    });
    expect(loadWarmRoster<RosterPayload>("movie:550:::q1080:full")?.result.streamUrl).toBe(
      "https://cdn.test/q.m3u8"
    );
    persistWarmRoster(
      "movie:550:::q1080:full",
      { streamUrl: "https://cdn.test/hd.m3u8", sources: [{ label: "Luna" }] },
      Date.now() + 60_000,
      1080
    );
    expect(loadWarmRoster<RosterPayload>("movie:550:::q2160:full")?.result.streamUrl).toBe(
      "https://cdn.test/q.m3u8"
    );
    expect(loadWarmRoster<RosterPayload>("movie:550:::q1080:full")?.result.streamUrl).toBe(
      "https://cdn.test/hd.m3u8"
    );
  ...
  it("persists HD-only results under q1080 even when the request was 4K", () => {
    isolatedDir();
    persistWarmRoster(
      "movie:550:::q2160:fast",
      { streamUrl: "https://cdn.test/hd.m3u8", sources: [{ label: "Luna" }] },
    ...
    expect(rosterIdentityForHeight("movie:550:::q2160:fast", 1080)).toBe(
      "movie-550:q1080"
    );
    expect(loadWarmRoster<RosterPayload>("movie:550:::q2160:fast")).toBeNull();
    expect(loadWarmRoster<RosterPayload>("movie:550:::q1080:full")?.result).toEqual({
      streamUrl: "https://cdn.test/hd.m3u8",
      sources: [{ label: "Luna" }],
    });
  });

  it("lets an HD request reuse a 4K disk roster", () => {
    isolatedDir();
    persistWarmRoster(
      "movie:550:::q1080:full",
      { streamUrl: "https://cdn.test/q.m3u8", sources: [{ label: "Quasar" }] },
    ...
    expect(rosterIdentityForHeight("movie:550:::q1080:full", 2160)).toBe(
      "movie-550:q2160"
    );
    expect(loadWarmRoster<RosterPayload>("movie:550:::q2160:fast")?.result.streamUrl).toBe(
      "https://cdn.test/q.m3u8"
    );
    expect(loadWarmRoster<RosterPayload>("movie:550:::q1080:full")?.result.streamUrl).toBe(
      "https://cdn.test/q.m3u8"
