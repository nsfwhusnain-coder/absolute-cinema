/**
 * Absolute Cinema Stream Scraper — multi-provider stream resolver.
...
/** Keep hunting until this many unique servers (roster). */
...
function providerFromEmbed(embedUrl: string): string {
  try {
    ...
    // Distinct Vidsrc mirrors → distinct city chips after label map.
...
async function applyLatencyProbes(
  ...
  // roster: keep probe-failed + unprobed for manual server switching.
...
function cacheTtlForResult(result: ScrapeResult): number {
  ...
  if (rosterHasPlayableHeight(result.sources, PLAYABLE_UHD_HEIGHT)) {
    return Math.max(base, FOUR_K_ROSTER_TTL_MS);
  }
  return base;
}

function persistDiskRoster(key: string, result: ScrapeResult, ramTtlMs: number): void {
  if (ramTtlMs <= 0) return;
  const diskTtlMs =
    ramTtlMs > ULTRA_MISS_TTL_MS
      ? Math.max(ramTtlMs, DISK_ROSTER_TTL_MS)
      : ramTtlMs;
  persistWarmRoster(
    key,
    result,
    Date.now() + diskTtlMs,
    rosterMaxPlayableHeight(result.sources)
  );
}
...
function rememberTitleRoster(key: string, result: ScrapeResult, ramTtlMs: number): void {
  ...
  persistDiskRoster(key, result, ramTtlMs);
}

/** Disk/sibling reuse: a q2160 identity must actually contain playable 4K. */
function cachedResultFitsRequest(key: string, result: ScrapeResult): boolean {
  if (!result.sources.length) return false;
  if (qualityBucketFromCacheKey(key) !== ROSTER_BUCKET_UHD) return true;
  return rosterHasPlayableHeight(result.sources, PLAYABLE_UHD_HEIGHT);
}
...
function findCachedSibling(key: string): ScrapeResult | null {
  ...
  for (const [otherKey, entry] of resultCache) {
    ...
    if (!cachedResultFitsRequest(key, entry.result)) continue;
    return entry.result;
  }
  const disk = loadWarmRoster<ScrapeResult>(key);
  if (disk && cachedResultFitsRequest(key, disk.result)) return disk.result;
  return null;
}
...
function getCached(key: string): ScrapeResult | null {
  ...
  const disk = loadWarmRoster<ScrapeResult>(key);
  if (disk && cachedResultFitsRequest(key, disk.result)) {
...
/** CinePro (OMSS) — multi-provider + proxy; fan-out when CINEPRO_URL set. */
async function resolveCineproEntries(
  ...
  // home /api/hls. Keep the full list; auto-default still prefers
...
async function enrichMissingSources(
  ...
  // Skip BOTH PW waves when 4K is already in hand (including late API
  // rows) or the measured seed is already healthy. Vidking PW is often
  // ~17s (over budget) and burns the only browser (pool size 1).
  // Threshold = VERIFIED_MIN_SKIP_SECONDARY (4) measured-playable
  // non-poison sources — Witcher S1E1: Luna+Quasar+Rock×3 in 921ms.
  // Vidrock stays on the API race; it is never skipped here.
  if (rosterHasPlayableHeight(mergeSourceEntries(result), PLAYABLE_UHD_HEIGHT)) {
    logAt("info", `[scrape] PW skipped (4K already in hand)`);
    return result;
  }
