...
export function rosterIdentity(titleOrCacheKey: string): string {
  return `${titleIdForRoster(titleOrCacheKey)}:${qualityBucketFromCacheKey(titleOrCacheKey)}`;
}

/** Disk identity from the roster's actual playable height, not the request hint. */
export function rosterIdentityForHeight(
  titleOrCacheKey: string,
  height: number
): string {
  return `${titleIdForRoster(titleOrCacheKey)}:${qualityBucketFromHeight(height)}`;
}
...
function titleIdForRoster(titleOrCacheKey: string): string {
  return (
...
export function persistWarmRoster<T>(
  ...
  const titleId =
    resultMaxHeight != null && Number.isFinite(resultMaxHeight)
      ? rosterIdentityForHeight(titleOrCacheKey, resultMaxHeight)
      : rosterIdentity(titleOrCacheKey);
  writeJson(rosterPath(titleId), { key: titleId, expiresAt, result });
  pruneOldest(rosterDir(), MAX_ROSTER_FILES);
}

function loadRosterFile<T>(identity: string): WarmRoster<T> | null {
  const parsed = readJson<WarmRoster<T>>(rosterPath(identity));
  if (!parsed?.key || typeof parsed.expiresAt !== "number") return null;
  if (parsed.expiresAt <= Date.now()) {
    try {
      unlinkSync(rosterPath(identity));
    } catch {
      /* ignore */
    }
    return null;
...
export function loadWarmRoster<T>(titleOrCacheKey: string): WarmRoster<T> | null {
  const direct = loadRosterFile<T>(rosterIdentity(titleOrCacheKey));
  if (direct) return direct;
  if (qualityBucketFromCacheKey(titleOrCacheKey) !== ROSTER_BUCKET_HD) return null;
  return loadRosterFile<T>(
    `${titleIdForRoster(titleOrCacheKey)}:${ROSTER_BUCKET_UHD}`
  );
}
