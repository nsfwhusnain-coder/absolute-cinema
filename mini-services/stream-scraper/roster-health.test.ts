...
describe("playable roster completion", () => {
  ...
  it("skips Playwright once a playable 4K source is already in hand", () => {
    expect(
      shouldSkipPlaywrightForHealthyRoster([
        { url: "https://cdn.example/uhd.m3u8", verified: true, maxHeight: 2160 },
      ])
    ).toBe(true);
    expect(
      shouldSkipPlaywrightForHealthyRoster([
        { url: "https://cdn.example/hd.m3u8", verified: true, maxHeight: 1080 },
      ])
    ).toBe(false);
  });

  it("reports the highest playable height for disk roster identity", () => {
    expect(
      rosterMaxPlayableHeight([
        { url: "https://cdn.example/hd.m3u8", verified: true, maxHeight: 1080 },
        { url: "https://cdn.example/uhd.m3u8", verified: true, ladder: [2160, 1080] },
        { url: "https://cdn.example/dead.m3u8", probe: { ok: false }, maxHeight: 2160 },
      ])
    ).toBe(2160);
  });
});
