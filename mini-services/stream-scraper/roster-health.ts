...
export const PLAYABLE_UHD_HEIGHT = 2160;
...
export function rosterMaxPlayableHeight(
  sources: RosterHealthSource[]
): number {
  let max = 0;
  for (const source of sources) {
    if (!isAutoPlayable(source)) continue;
    max = Math.max(max, source.maxHeight ?? 0, source.ladder?.[0] ?? 0);
  }
  return max;
}
...
export function shouldSkipPlaywrightForHealthyRoster(
  ...
  if (rosterHasPlayableHeight(sources, PLAYABLE_UHD_HEIGHT)) return true;
  return (
