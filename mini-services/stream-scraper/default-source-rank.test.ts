...
describe("sortSourcesForDefault — R9 height tiers", () => {
  ...
  it("3b: qualityHint 2160 prefers 4K among HD sources", () => {
    const s1080 = src({ id: "s1080", maxHeight: 1080 });
    const s2160 = src({ id: "s2160", maxHeight: 2160 });
    const ranked = sortSourcesForDefault([s1080, s2160], {
  ...
  it("unprobed 2160 beats probe.ok 1080 when both exist", () => {
    const hd = src({
      id: "hd",
      maxHeight: 1080,
      probe: { ok: true, speedScore: 90 },
    });
    const uhd = src({ id: "uhd", maxHeight: 2160 });
    expect(sortSourcesForDefault([hd, uhd])[0]?.id).toBe("uhd");
    expect(
      sortSourcesForDefault([hd, uhd], { qualityHintHeight: 1080 })[0]?.id
    ).toBe("uhd");
  });
